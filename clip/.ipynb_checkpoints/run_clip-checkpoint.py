...
sys.path.append(currentpath.split("clip")[0])
...
if __name__ == '__main__':
    ...
    for kb_name in args.kbs:
        ...
        rho = ExpressRefinement(knowledge_base=kb, use_inverse=False, use_numeric_datatypes=False)
        ...
        model = CLIP(knowledge_base=kb, path_of_embeddings=f"../clip_weights/{kb_name}/embeddings/{kb_name}.csv", refinement_operator=rho, load_pretrained=True, max_runtime=300)
        ...
        with open(f'../datasets/{kb_name}/Results/CLIP_results.json', 'w') as file_descriptor1:
                    json.dump(Result_dict, file_descriptor1, ensure_ascii=False, indent=3)

        with open(f'../datasets/{kb_name}/Results/CLIP_avg_results.json', 'w') as file_descriptor2:
