...
if __name__ == '__main__':
    ...
    for num_examples in tqdm(get_lp_size(max_lp_size), desc="Next example's set size"):
        durations = []
        predictions = []
        model = CLIP(knowledge_base=KB, path_of_embeddings=f"../clip_weights/{kb}/embeddings/{kb}.csv", refinement_operator=rho, load_pretrained=True, max_runtime=300)
    ...
    file_name = f"../datasets/CLIP_{kb}_incremental.json"
