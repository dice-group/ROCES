...
sys.path.append(currentpath.split("drill")[0])
...
if __name__ == '__main__':
    ...
    for kb_name in args.kbs:
        ...
        ## Model
        model = Drill(knowledge_base=kb, path_pretrained_kge=f"../entity_embs/{kb_name}.csv",
                  quality_func=F1(), max_runtime=300)
        for str_target_concept, examples in tqdm(lps, desc=f'Learning {len(lps)} problems'):
            #model = Drill(knowledge_base=kb, refinement_operator=rho, quality_func=F1(), path_of_embeddings=f"../entity_embs/{kb_name}.csv", max_runtime=300)
        ...
        with open(f'../datasets/{kb_name}/Results/Drill_results.json', 'w') as file_descriptor1:
                    json.dump(Result_dict, file_descriptor1, ensure_ascii=False, indent=3)

        with open(f'../datasets/{kb_name}/Results/Drill_avg_results.json', 'w') as file_descriptor2:
